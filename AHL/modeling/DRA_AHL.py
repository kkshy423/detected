...
class DRA(nn.Module):
    def __init__(self, cfg, backbone="resnet18", flag = None):
        ...
        self.vars.append(nn.Parameter(torch.zeros([512])))
        self.vars.append(nn.Parameter(torch.zeros([512])))
        self.register_buffer("bn_running_mean", torch.zeros([512]), persistent=False)
        self.register_buffer("bn_running_var", torch.ones([512]), persistent=False)
    ...
    def forward(self, image=None, image_scale=None, label= None, var = None, st =False):
        ...
        for s in range(self.cfg.n_scales):
            ...
            if self.training and st == False:
                ...
                # comparison_scores
                ref = torch.mean(ref_feature, dim=0).expand(feature.size(0), -1, -1, -1)
                temp = ref - feature
                x = temp
                w, b = var[10], var[11]
                x = F.conv2d(x, w, b, stride=3, padding=0)
                w, b = var[12], var[13]
                x = F.batch_norm(x, self.bn_running_mean, self.bn_running_var, weight=w, bias=b)
                x = F.relu(x)
                w, b = var[8], var[9]
