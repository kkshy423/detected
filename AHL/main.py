...
class Trainer(object):
    def __init__(self, args):
        ...
        if self.args.auxiliary == True:
            self.aux_model = AUX_Model(self.args.episode_num)
    ...
    def _phase_mean(self, batch_times, key):
        values = [float(row[key]) for row in batch_times if key in row and row[key] is not None]
        return self._mean(values)

    def _write_ahl_runtime(self, eval_name, batch_times, sample_count):
        elapsed_ms = sum(float(row["elapsed_ms"]) for row in batch_times)
        sample_count = int(sample_count)
        elapsed_values = []
        for row in batch_times:
            batch_size = max(int(row.get("batch_size", 1)), 1)
            elapsed_per_sample = float(row["elapsed_ms"]) / batch_size
            row.setdefault("elapsed_per_sample_ms", elapsed_per_sample)
            elapsed_values.append(elapsed_per_sample)
        mean_ms = elapsed_ms / sample_count if sample_count > 0 else None
        median_ms = self._percentile(elapsed_values, 0.50)
        p90_ms = self._percentile(elapsed_values, 0.90)
        p95_ms = self._percentile(elapsed_values, 0.95)
        trimmed_values = sorted(elapsed_values)
        if len(trimmed_values) >= 20:
            trim = max(1, int(len(trimmed_values) * 0.05))
            trimmed_values = trimmed_values[trim:-trim]
        payload = {
            "eval_name": eval_name,
            "time_kind": "single_image_ahl_process_mean_ms",
            "time_ahl_process_ms": mean_ms,
            "ahl_process_time_ms": mean_ms,
            "time_ahl_process_median_ms": median_ms,
            "time_ahl_process_p90_ms": p90_ms,
            "time_ahl_process_p95_ms": p95_ms,
            "time_ahl_process_trimmed_mean_ms": self._mean(trimmed_values),
            "time_ahl_process_drop_first_mean_ms": self._mean(elapsed_values[1:]),
            "preload_ref_eval": bool(getattr(self.args, "preload_ref_eval", False)),
            "production_like_fast_path": bool(getattr(self.args, "production_like_fast_path", False)),
            "legacy_eval_path": not bool(getattr(self.args, "production_like_fast_path", False)),
            "result_dump_disabled": bool(getattr(self.args, "disable_result_dump", False)),
            "sample_count": sample_count,
            "batch_count": len(batch_times),
            "elapsed_ms": elapsed_ms,
            "phase_mean_ms": {
                "input_to_device": self._phase_mean(batch_times, "input_to_device_ms"),
                "reference_attach": self._phase_mean(batch_times, "reference_attach_ms"),
                "ahl_forward": self._phase_mean(batch_times, "ahl_forward_ms"),
                "score_postprocess": self._phase_mean(batch_times, "score_postprocess_ms"),
            },
            "top_slowest_batches": sorted(
                batch_times, key=lambda row: float(row["elapsed_ms"]), reverse=True
            )[:20],
            "batch_times": batch_times,
            "note": (
                "legacy_eval_path includes the historical evaluation loop. "
                "production_like_fast_path starts after the test dataloader yields "
                "precomputed ADPretrain feature tensors, uses cached reference "
                "batches when enabled, excludes loss/metric/result-file work from "
                "the timed region, and records CPU score transfer after timing."
            ),
        }
        path = os.path.join(self.args.experiment_dir, "ahl_runtime.json")
        with open(path, "w", encoding="utf-8") as f:
            json.dump(payload, f, ensure_ascii=False, indent=2)

    def _preload_eval_ref_batches(self):
        if self.args.model_name != "DRA" or self.args.total_heads != 4:
            return
        self.eval_ref_batches = []
        for sample in self.ref_loader:
            ref_image = sample['image']
            ref_image_scale = sample['image_scale']
            if self.args.cuda:
                ref_image = ref_image.cuda()
                ref_image_scale = ref_image_scale.cuda()
            self.eval_ref_batches.append((ref_image, ref_image_scale))
        if not self.eval_ref_batches:
            raise RuntimeError("No reference batches available for eval preload")
        self._sync_if_cuda()

    def _next_eval_ref_batch(self, batch_idx=None):
        if getattr(self.args, "preload_ref_eval", False) and batch_idx is not None:
            if not hasattr(self, "eval_ref_batches"):
                self._preload_eval_ref_batches()
            return self.eval_ref_batches[batch_idx % len(self.eval_ref_batches)]
        try:
            sample = next(self.ref)
        ...
        if self.args.cuda:
            ref_image = ref_image.cuda()
            ref_image_scale = ref_image_scale.cuda()
        return ref_image, ref_image_scale

    def _sample_names(self, sample):
        names = sample.get("fn", None)
        if names is None:
            return []
    ...
    def _write_result(self, total_target, total_pred):
        if getattr(self.args, "disable_result_dump", False):
            return
    ...
    def _concat_or_empty(self, values):
        if not values:
            return np.array([])
        return np.concatenate(values, axis=0)

    def _combine_dra_scores(self, outputs):
        total_pred = -1 * outputs[0]
        for i in range(1, self.args.total_heads):
            total_pred = total_pred + outputs[i]
        return total_pred
    ...
    def train_unit(self, image, image_scale, image_targets, var = None, st = False):
        if self.args.model_name == "DRA":
            if self.args.total_heads == 4:
                ref_image, ref_image_scale = self._next_eval_ref_batch()
    ...
    def eval_DRA(self):
        if getattr(self.args, "production_like_fast_path", False):
            return self.eval_DRA_fast()
        self.model.eval()
        tbar = self.test_loader if getattr(self.args, "disable_tqdm", False) else tqdm(self.test_loader, desc='\r')
        test_loss = 0.0
        class_pred = [[] for _ in range(self.args.total_heads)]
        total_target = []
        batch_times = []
        for batch_idx, sample in enumerate(tbar):
            ...
            if self.args.total_heads == 4:
                ref_image, ref_image_scale = self._next_eval_ref_batch(batch_idx)
                image = torch.cat([ref_image, image], dim=0)
                image_scale = torch.cat([ref_image_scale, image_scale], dim=0)
            ...
            test_loss += loss.item()
            if not getattr(self.args, "disable_tqdm", False):
                tbar.set_description('Test loss: %.3f' % (test_loss / (batch_idx + 1)))
            total_target.append(target.cpu().numpy().reshape(-1))

            for i in range(self.args.total_heads):
                if i == 0:
                    data = -1 * outputs[i].detach().cpu().numpy()
                else:
                    data = outputs[i].detach().cpu().numpy()
                class_pred[i].append(data.reshape(-1))
            self._sync_if_cuda()
            batch_times.append({
                "batch_index": int(batch_idx),
                "batch_size": int(target.shape[0]),
                "elapsed_ms": float((time.perf_counter() - batch_start) * 1000.0),
                "sample_names": self._sample_names(sample),
            })

        total_target = self._concat_or_empty(total_target)
        class_pred = [self._concat_or_empty(pred) for pred in class_pred]
        total_pred = self.normalization(class_pred[0])
        for i in range(1, self.args.total_heads):
            total_pred = total_pred + self.normalization(class_pred[i])

        self._write_result(total_target, total_pred)
        self._write_ahl_runtime("eval_DRA", batch_times, len(total_target))
    ...
    def eval_DRA_fast(self):
        self.model.eval()
        class_pred = []
        total_target = []
        batch_times = []
        if getattr(self.args, "preload_ref_eval", False):
            self._preload_eval_ref_batches()

        iterator = self.test_loader
        if not getattr(self.args, "disable_tqdm", False):
            iterator = tqdm(iterator, desc='production-like eval')
        for batch_idx, sample in enumerate(iterator):
            ...
            if self.args.cuda:
                ...
                image_scale = image_scale.cuda(non_blocking=True)
            self._sync_if_cuda()
            ...
            with torch.no_grad():
                outputs = self.model.forward(
                    ...
                    label=None,
                    var=self.model.parameters(),
                )
            self._sync_if_cuda()
            after_forward = time.perf_counter()

            score = self._combine_dra_scores(outputs)
            self._sync_if_cuda()
            after_score = time.perf_counter()

            class_pred.append(score.detach().cpu().numpy().reshape(-1))
            total_target.append(target.numpy().reshape(-1))
            batch_times.append({
                "batch_index": int(batch_idx),
                "batch_size": int(target.shape[0]),
                "elapsed_ms": float((after_score - batch_start) * 1000.0),
                "input_to_device_ms": float((after_input - batch_start) * 1000.0),
                "reference_attach_ms": float((after_ref - after_input) * 1000.0),
                "ahl_forward_ms": float((after_forward - after_ref) * 1000.0),
                "score_postprocess_ms": float((after_score - after_forward) * 1000.0),
                "sample_names": self._sample_names(sample),
            })

        total_pred = self.normalization(self._concat_or_empty(class_pred))
        total_target = self._concat_or_empty(total_target)
        self._write_result(total_target, total_pred)
        self._write_ahl_runtime("eval_DRA_fast", batch_times, len(total_target))
    ...
    def eval_devnet(self):
        ...
        for batch_idx, sample in enumerate(tbar):
            ...
            total_pred.append(data.reshape(-1))
            total_target.append(target.cpu().numpy().reshape(-1))
            self._sync_if_cuda()
            batch_times.append({
                "batch_index": int(batch_idx),
        ...
        total_pred = self._concat_or_empty(total_pred)
        total_target = self._concat_or_empty(total_target)
        self._write_result(total_target, total_pred)
        self._write_ahl_runtime("eval_devnet", batch_times, len(total_target))
    ...
    def eval_devnet_fast(self):
        ...
        for batch_idx, sample in enumerate(iterator):
            ...
            total_pred.append(score.detach().cpu().numpy().reshape(-1))
            total_target.append(target.numpy().reshape(-1))
            batch_times.append({
                "batch_index": int(batch_idx),
                "batch_size": int(target.shape[0]),
                "elapsed_ms": float((after_score - batch_start) * 1000.0),
                "input_to_device_ms": float((after_input - batch_start) * 1000.0),
                "ahl_forward_ms": float((after_forward - after_input) * 1000.0),
                "score_postprocess_ms": float((after_score - after_forward) * 1000.0),
                "sample_names": self._sample_names(sample),
            })
        ...
        total_target = self._concat_or_empty(total_target)
        self._write_result(total_target, total_pred)
        self._write_ahl_runtime("eval_devnet_fast", batch_times, len(total_target))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval_only', action='store_true', help="skip training and only run one evaluation pass")
    parser.add_argument('--load_weights', type=str, default=None, help="path to trained model weights to load")
    parser.add_argument('--preload_ref_eval', action='store_true', help="cache reference feature batches before eval-only timing")
    parser.add_argument('--production_like_fast_path', '--fast_infer', action='store_true',
                        help="use production-like eval timing: cached ref, no loss/tqdm/write work in timed region")
    parser.add_argument('--disable_result_dump', action='store_true',
                        help="do not write result.txt; intended for latency-only production-like runs")
    parser.add_argument('--keep_fast_result_dump', action='store_true',
                        help="when production_like_fast_path is enabled, still write result.txt after timing")
    parser.add_argument('--disable_tqdm', action='store_true',
                        help="disable tqdm progress display during eval loops")
    args = parser.parse_args()

    if args.production_like_fast_path:
        args.preload_ref_eval = True
        args.disable_tqdm = True
        if not args.keep_fast_result_dump:
            args.disable_result_dump = True
    ...
    if args.eval_only and not weight_path:
        weight_path = os.path.join(args.experiment_dir, args.classname + "_ctest.pkl")
    if weight_path:
        ...
        print("Load model weight from: " + weight_path)
