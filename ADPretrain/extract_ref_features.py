...
def main(args):
    ...
    for class_name in DATASET_CLASSES[dataset].CLASS_NAMES:
        ...
        runtime = {
            ...
            'time_adpretrain_feature_ms': float(sum(elapsed) / sample_count) if sample_count else None,
            'sample_count': sample_count,
            'batch_count': len(elapsed),
            'note': 'Timer covers encoder.encode_image_from_tensors for reference feature extraction only.',
        }
        with open(os.path.join(save_dir, class_name, 'runtime.json'), 'w', encoding='utf-8') as f:
