#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate fixed-test and staged train-only splits for models_qiumianxiepai."""
import argparse
import random
...
DEFAULT_OUTPUT_ROOT = Path("/ghome/huangjd/code/detected/adpretrain_bridge/splits/qm_xiepai_fewshot_train_test")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare qm_xiepai train/test-only few-shot split files.")
...
def main() -> None:
    ...
    for stage, spec in STAGE_SPECS.items():
        train_normal = production_normal_order[: spec.available_normal]
        train_anomaly = production_anomaly[: spec.available_anomaly]
        if len(train_normal) != spec.available_normal:
            raise ValueError(f"Normal count mismatch for {stage}")
        if len(train_anomaly) != spec.available_anomaly:
        ...
        stage_specs[stage] = {
            ...
            "test_normal": args.test_normal_count,
            "test_anomaly": args.test_anomaly_count,
    ...
    config = {
        ...
        "notes": [
            "No calibration files are generated.",
            "Each stage trains on all currently available production normal and anomaly samples.",
            ...
            "Stage training sets are cumulative prefixes of the production pools.",
        ],
    }
    write_json(output_root / "split_config.json", config)
    print(f"Train/test split generation OK: {output_root}")
